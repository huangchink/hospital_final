...
+ always show both circle outlines (one with stimulus fill, one only border)
"""
...
class Staircase:
    """Simple up-down staircase with reversal & correct-streak rules."""
    ...
    def update(self, correct):
        ...
        if correct:
            ...
            self.max_correct_streak = max(self.max_correct_streak,
                                          self.correct_streak)
        else:
            self.correct_streak = 0
    ...
    def done(self):
        # finish if ≥4 reversals or top freq + ≥3 correct in a row
...
class SettingsWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.stim_dur = QDoubleSpinBox()
        self.stim_dur.setRange(0.5, 10.0)
        self.stim_dur.setValue(2.0)
        layout.addWidget(self.stim_dur)

        add_label("Blank Duration (s):")
        self.blank_dur = QDoubleSpinBox()
        self.blank_dur.setRange(0.5, 10.0)
        self.blank_dur.setValue(1.0)
        layout.addWidget(self.blank_dur)

        add_label("Circle Radius (px):")
        self.rad = QDoubleSpinBox()
        self.rad.setRange(50, 500)
        self.rad.setValue(250)
        layout.addWidget(self.rad)

        add_label("Threshold (px):")
        self.thresh = QDoubleSpinBox()
        self.thresh.setRange(10, 500)
        self.thresh.setValue(250)
...
def run_test(cfg):
    ...
    while not stair.done():
        ...
        while True:
            ...
            # draw background + stimulus masked
            ...
            # overlay both circle outlines (white border only)
            pygame.draw.circle(win, (255,255,255), centers['left'],  cfg['rad'], 2)
            pygame.draw.circle(win, (255,255,255), centers['right'], cfg['rad'], 2)

            # get gaze & sustained-hold logic
            gi = gf.get_gaze_info()
            dist = float('inf')
